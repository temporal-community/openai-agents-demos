...
class ResearchWorkflow:
    ...
    @workflow.run
    async def run(self, initial_query: str | None = None, use_clarifications: bool = False) -> str:
        """
        ...
        if initial_query and not use_clarifications:
            ...
            return await self.research_manager.run(initial_query, use_clarifications=False)
        
        # Long-running workflow - wait for termination or completion like wealth management
        await workflow.wait_condition(
            lambda: bool(self._end_workflow or (
                self.current_interaction and 
        ...
        return self.current_interaction.final_result or "No research completed" if self.current_interaction else "No research completed"
    ...
    async def _complete_research_with_clarifications(self):
        ...
        if not self.current_interaction or not self.current_interaction.clarification_responses:
            return
        ...
        final_result = await self.research_manager.run_with_clarifications_complete(
            self.current_interaction.original_query,
            questions,
            responses
        )
    ...
    @workflow.update  
    async def provide_single_clarification(self, input: SingleClarificationInput) -> ResearchInteraction:
        """Provide a single clarification response"""
        if not self.current_interaction:
            raise ValueError("No active research interaction")
        
        if self.current_interaction.status not in ["awaiting_clarifications", "collecting_answers"]:
            raise ValueError(f"Not collecting clarifications. Current status: {self.current_interaction.status}")
    ...
    async def provide_clarifications(self, input: ClarificationInput) -> ResearchInteraction:
        ...
        self.current_interaction.current_question_index = len(self.current_interaction.clarification_questions or [])
